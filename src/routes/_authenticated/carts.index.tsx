...
function CartsTable({ filtered, setViewId }: { filtered: any[]; setViewId: (id: string) => void }) {
        ...
        {cart && (
          <>
            ...
            <div className="mt-4">
              ...
              <CartDocumentsTable key={cart.id} docs={docs} />
            </div>
...
function CartDocumentsTable({ docs }: { docs: any[] }) {
  const { page, setPage, totalPages, paged, pageSize, total } = usePagination(docs, 5);
            ...
            {paged.length ? (
              paged.map((d) => (
                <tr key={d.id} className="hover:bg-slate-50/60 transition-colors">
                  <td className="px-3 py-2 font-medium text-slate-800">{d.document_name}</td>
